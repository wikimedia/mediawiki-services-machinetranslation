...
if [ -d "${MODEL_DIR}" ] && [ "$(ls -A $MODEL_DIR)" ]; then
	...
	wget -N --inet4-only --no-verbose --show-progress --progress=bar:force:noscroll "${MODEL_BASE_URL}/nllb200-600M.tgz" -P $BASE_MODEL_DIR
...
do
	if [ -f "${MODEL_DIR}/${i}" ]; then
		...
		wget -N --inet4-only --no-verbose --show-progress --progress=bar:force:noscroll "${MODEL_BASE_URL}"/${i} -P $MODEL_DIR
	fi
done
...
if [ -d "${MODEL_DIR}" ] && [ "$(ls -A $MODEL_DIR)" ]; then
	...
	wget -N --inet4-only --no-verbose --show-progress --progress=bar:force:noscroll "${MODEL_BASE_URL}/madlad400-3b-ct2.tgz" -P $BASE_MODEL_DIR
	pushd $BASE_MODEL_DIR
