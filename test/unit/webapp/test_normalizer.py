...
def test_normalize_fr():
    ...
    assert normalize("fr", "«La vie est belle» disait ma grand-mère.") == '"La vie est belle" disait ma grand-mère.'
    assert normalize("fr", "J'ai visité la «Tour Eiffel» aujourd'hui.") == 'J\'ai visité la "Tour Eiffel" aujourd\'hui.'
    assert normalize("fr", "Pourquoi ne peut-on pas simplement dire «non»?") == "Pourquoi ne peut-on pas simplement dire \"non\"?"


def test_normalize_de():
    assert normalize("de", '„Guten Tag“, sagte er.') == '"Guten Tag", sagte er.'
    assert normalize("de", '„Das ist ein Test“ beantwortete sie.') == '"Das ist ein Test" beantwortete sie.'
    assert normalize("de", 'Ich mag „Berlin“ viel mehr als „München“.') == 'Ich mag "Berlin" viel mehr als "München".'


def test_normalize_other():
    assert normalize("es", "Hola. ¿Cómo estás?") == "Hola. ¿Cómo estás?"  # Test non-supported language
