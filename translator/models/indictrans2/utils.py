"""
Preprocessing, postprocessing and normalization utilities for
indictrans2 modules. Adapted from
https://github.com/AI4Bharat/IndicTrans2/blob/main/inference/engine.py
MIT license
"""
...
def normalize(
    ...
    the Indic numerals in the input string to Roman script. Later, it uses the input
    string with normalized Indic numerals to wrap the spans of text matching the pattern
...
def preprocess_sent(
    ...
    Args:
        ...
        normalizer (Union[MosesPunctNormalizer, indic_normalize.IndicNormalizerFactory]): an object
        that performs normalization on the text.
        lang (str): flores language code of the input text sentence.

    Returns:
        Tuple[str, dict]: a tuple of preprocessed input text sentence and also
        a corresponding dictionary mapping placeholders to their original values.
    """
...
def preprocess(sents: List[str], lang: str):
    ...
    Returns:
        Tuple[List[str], List[dict]]: a tuple of list of preprocessed input text sentences and also
        a corresponding list of dictionary mapping placeholders to their original values.
    """
...
def preprocess_batch(batch: List[str], src_lang: str) -> List[str]:
    """
    Preprocess an array of sentences by normalizing, tokenization, and possibly transliterating it.
    It also tokenizes the normalized text sequences using sentence piece tokenizer and also adds
    language tags.
    ...
    return preprocessed_sents, placeholder_entity_map_sents
...
def postprocess_batch(
    ...
    Args:
        ...
        placeholder_entity_map (List[dict]): dictionary mapping placeholders
        to the original entity values.
        lang (str): flores language code of the input sentences.
        common_lang (str, optional): flores language code of
        the transliterated language (defaults: hin_Deva).
    ...
    for i in range(len(sents)):
        ...
        if script_code in {"Arab", "Aran"}:
            # UrduHack adds space before punctuations. Since the model was
            # trained without fixing this issue, let's fix it now
        ...
        # Oriya bug: indic-nlp-library produces ଯ଼ instead of ୟ when converting from
        # Devanagari to Odia
    ...
    if lang == "eng_Latn":
        ...
        postprocessed_sents = [
            indic_detokenize.trivial_detokenize(
                xliterator.transliterate(s, flores_codes[common_lang], flores_codes[lang]),
                flores_codes[lang],
            )
            for s in sents
        ]
